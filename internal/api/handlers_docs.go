...
<li>Webhook and <code>notify_url</code> URLs are checked for literal private/loopback/link-local addresses only at registration; a hostname is not resolved at that point. The actual delivery and notify fetches go through a dial-time guard that rejects a resolved private/loopback/link-local address on every connection attempt (so a hostname that resolves to one still fails, just later &mdash; the delivery's <code>last_error</code> reads "not a public address"), and never follow a redirect. Redirect URLs are not fetched by this service at all &mdash; only the end user's browser ever follows one &mdash; so they get no SSRF check; what constrains them is the redirect-domain allowlist above.</li>
...
<li>With <code>DB_DRIVER=sqlite</code> (the default), storage is a single-connection file sized for one process and one disk &mdash; fine for local dev and tests, not for a public deployment. Set <code>DB_DRIVER=postgres</code> for a real connection pool (see the README's "Setup" &sect;7).</li>
